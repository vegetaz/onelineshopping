...
import mnm.n13.onlineshopping.model.CheckoutModel;
import mnm.n13.onlineshopping.model.UserModel;
...
import mnm.n13.shoppingbackend.dto.OrderItem;
...
public class CheckoutHandler {
	...
	public CheckoutModel init(String name) throws Exception{
		...
		if(user!=null) {
			...
			if(cartLines.size() == 0 ) {
				throw new Exception("Hiện tại không có sản phẩm nào để thanh toán!");
			}
	...
	public String saveOrder(CheckoutModel checkoutModel) {
		...
		//set path
		if(path != null) {
			orderDetail.setImagePath(path);
			if(path.equals("-1")) {
				boolean check = cartLineDAO.updateAmountPay(checkoutModel.getUser().getEmail(),checkoutModel.getCheckoutTotal());
				if(check == false) {
					return "error";
				}
			}
