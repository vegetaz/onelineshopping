$(function() {
	...
	if($productsTable.length) {
		...
		$productsTable.DataTable({
					lengthMenu : [ [ 10, 30, 50, -1 ], [ '10', '30', '50', 'Tất cả' ] ],
					pageLength : 30,
					ajax : {
						url : jsonUrl,
						dataSrc : ''
					...
					initComplete: function () {
						...
						api.$('.switch input[type="checkbox"]').on('change' , function() {							
							var dText = (this.checked)? 'Kích hoạt sản phẩm?': 'Huỷ kích hoạt sản phẩm?';
							var checked = this.checked;
							var checkbox = $(this);
							debugger;
						    bootbox.confirm({
						    	size: 'medium',
						    	title: 'Sản phẩm kích hoạt/không kích hoạt',
						    	message: dText,
		...
		$categoryForm.validate({			
				...
				messages: {					
					name: {
						...
						minlength: 'Vui lòng nhập ít nhất năm ký tự'
					},
					description: {
						required: 'Nhập tên sản phẩm!',
						minlength: 'Vui lòng nhập ít nhất năm ký tự'
		...
		$loginForm.validate({			
				...
				messages: {					
					username: {
						...
						email: 'Nhập một địa chỉ thư điện tử hợp lệ!'
					},
					password: {
		...
		if(countField.val() !== originalCount) {	
			...
			if(countField.val() < 1 || countField.val() > 3) {
				...
				bootbox.alert({
					size: 'medium',
			    	title: 'Lỗi',
			    	message: 'Số lượng sản phẩm nên nhỏ nhất là 1 và lớn nhất là 3!'
				});
			}
			else {
